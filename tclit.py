...
class LabelingTool:
    ...
    def setup_ui(self):
        ...
        ttk.Button(button_frame, text="Previous", command=self.prev_image).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Button(button_frame, text="Next", command=self.next_image).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Button(button_frame, text="Save", command=self.save_labels).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def load_current_image(self):
        ...
        if 0 <= self.current_idx < len(self.images):
            # Update progress
            self.progress_var.set(f"Image {self.current_idx + 1} of {len(self.images)}")
    ...
    def save_current_labels(self):
        ...
        if 0 <= self.current_idx < len(self.images):
            img_path = self.images[self.current_idx]
            selected = [
                category for category, var in self.var_dict.items() if var.get()
            ]
            if selected:
