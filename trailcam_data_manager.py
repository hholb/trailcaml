...
class TrailCamDataManager:
    ...
    def ingest_images(self, source_dir: Path, batch_name: Optional[str] = None):
        ...
        console.print(
            f"[green]Successfully ingested {len(image_files)} images to {batch_dir}"
        )
        return batch_dir
    ...
    def add_labels(self, image_labels: Dict[Path, List[str]]):
        ...
        """
        category_map = {
            cat["name"]: cat["id"] for cat in self.annotations["categories"]
        }

        for img_path, labels in track(
            image_labels.items(), description="Adding labels"
        ):
            img_id = self.generate_image_id(img_path)
            ...
            for label in labels:
                if label in category_map:
                    self.annotations["annotations"].append(
                        {"image_id": img_id, "category_id": category_map[label]}
                    )
                else:
    ...
    def distribute_to_splits(self):
        ...
        labeled_images = {ann["image_id"] for ann in self.annotations["annotations"]}
