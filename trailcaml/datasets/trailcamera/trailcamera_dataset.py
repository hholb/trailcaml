...
class TrailCameraDataset:
    def __init__(
        ...
        for split in ["train", "valid", "test"]:
            annotations_file = self.data_dir / split / annotations_filename
    ...
    def train(self) -> Dataset:
        return _TrailCameraDataset(
            "train", self.data_dir, self.train_annotations, self.size
        )

    @property
    def valid(self) -> Dataset:
        return _TrailCameraDataset(
            "valid", self.data_dir, self.valid_annotations, self.size
        )

    @property
    def test(self) -> Dataset:
        return _TrailCameraDataset(
            "test", self.data_dir, self.test_annotations, self.size
        )
...
class _TrailCameraDataset(Dataset):
    ...
    def _get_label_for_img(self, id: int) -> list[float]:
        antns = [an for an in self.annotations["annotations"] if an["image_id"] == id]
        animals = set(a["category_id"] for a in antns)
        categories = [c["id"] for c in self.annotations["categories"]]

        labels = [1.0 if c in animals else 0.0 for c in categories]
